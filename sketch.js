...
class layerSlider extends layerUI {
  constructor(layer, psoition, start, home, end, setting) {
    super(layer, psoition);
...
//Base class for objects that will colide
//Used only circle hitbox, and will only colide with objects passed in objects array
class collisionObject {
  ...
  //Functions that may be called but dont need to be defined
  hit() {
    return undefined;
  }
  physicsUpdate() {
...
//Class that handles some physics propertys like friction, ect. and also handles drawing and circling
class physicsObject extends collisionObject {
  constructor(position, radius, objects, friction) {
    ...
    this.drawFrame = true;
    //Adds object to objs
    this.convetedNum = stringToNum(this.constructor.name);
    Objs[this.convetedNum] = [this];
  }
  physicsUpdate() {
    this._physicsUpdate();
    //Calculates friction
...
class directionalPhysicsObject extends collisionObject {
  constructor(
    ...
    this.convetedNum = stringToNum(this.constructor.name);
    Objs[this.convetedNum].push(this);
  }
  physicsUpdate() {
    this._physicsUpdate();
    //Creates new movment vector so it doesent multiply
...
class ship extends physicsObject {
  ...
  _physicsUpdate() {
    ...
    //Handels key imputs for movment
    if (keyIsDown(87)) {
  ...
  hit() {
    ...
    //Sets flicker and invincability
...
//Bullet that moves in one direction and colides with rocks
...
//Gets a vector point in directiong of rotation
...
//Deletes object from obj object reming it from main loop in most cases
function deleteObj(instance) {
  let a = Objs[instance.convetedNum].indexOf(instance);
  delete Objs[instance.convetedNum][a];
  Objs[instance.convetedNum].splice(a, 1);
}
...
//Ends game by changing, clearing and seetting virables to prepare for next game and menu
function endGame() {
  gameRunning = false;
  deleteObj(mainShip);
  mainShip = undefined;
...
//Clears all objects for main objecst object
function clearObjs() {
  for (const property in Objs) {
    for (var i = 0; i < Objs[property].length; i++) {
      if (Objs[property][i] != null) {
...
function drawMenu() {
  ...
  //Draws sliders as they needed to be handeled differently
...
/*Updates settings when settings is left by going through all sliders and change the settings values to those of the sliders then changes off settigns layer*/
function settingsLeft(params) {
  for (var i = 0; i < sliderLayers[currentLayer].length; i++) {
    settings[sliderLayers[currentLayer][i].setting] = sliderLayers[
      currentLayer
    ][i].slider.value();
  }
  changeLayer(params);
...
//changes meanu layer given object with change property
...
function createMenuUI() {
  ...
  new layerText(
    1,
    createVector(width / 2, height / 2),
    "Destroy the astroids and dont get hit \n click to shoot \n W to move forward  \n D to spin right \n A to spin left \n DEL to end game",
    255,
    20
  );
}
